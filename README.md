...
Puesto que ir haciendo esto para cada comando que se quiera lanzar es un engorro, se subirá un archivo con una *reverse shell* a este directorio para después ejecutarlo desde este y tener acceso a la máquina. La *reverse shell* es descargada de la página de *Github* de [pentestmonkeys](https://github.com/pentestmonkey/php-reverse-shell) y se debe adecuar a la IP y puerto de la máquina atacante que escucha. Sólo es necesario modificar las variables **$ip** (en este caso 10.23.92.113) y **$port** (1234). Por un lado, se crea un servidor con *Python3* como se ha hecho en otros CTFs, desde la carpeta donde se almacena el archivo con la *reverse*.
